...
import { withNavigationFocus,NavigationEvents } from 'react-navigation';
  const a = "B-008 Auditorio"
  const b = 'Cood. Eventos'
  const c = "Xeroz"
  const d = 'Hall Principal'
  const e = 'Gremio'    
  const f = 'C.A'
  const g = 'Cantina'
  const h = 'Xadrez'
 ...
 class BBT1 extends React.Component {
    ...
    return (
      <View style={{marginLeft: '0%'}}>
        ...
        <View style={{marginTop: '0%'}}>
           ...
           <Text style={t.a5}> {e.slice(0,4) }</Text>
           <Text style={t.a5}> {e.slice(4,8)} </Text>
...
export default withNavigationFocus(BBT1)
